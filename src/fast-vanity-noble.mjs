...
ed.etc.sha512Sync = (...m) => {
  ...
  m.forEach(chunk => hash.update(chunk));
...
if (isMainThread) {
  ...
  const progressInterval = setInterval(() => {
    if (!found) {
      ...
      const perSec = elapsed > 0 ? Math.round(totalAttempts / elapsed) : 0;
      console.error(`[FastVanity] ${rate}M attempts (${elapsed}s, ${perSec}/s)`);
    }
  }, 5000);
  ...
  while (true) {
    for (let i = 0; i < 1000; i++) {
      ...
      const pubBase58 = toBase58(publicKey);
      
      if (pubBase58.endsWith(suffix)) {
        const fullSecret = new Uint8Array(64);
        fullSecret.set(privateKey);
        fullSecret.set(publicKey, 32);
        
        parentPort.postMessage({ 
          type: 'found', 
          publicKey: pubBase58, 
          secretKey: toBase58(fullSecret),
